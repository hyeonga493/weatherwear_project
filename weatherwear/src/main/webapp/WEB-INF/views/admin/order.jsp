...
<body>
	...
	<div id="wrap" style="height:2300px;">
		<div style="display: flex; justify-content: center;">
			<form id="dateSearchForm" name="dateSearchForm" action="/w2/adminOrder.mdo" method="post" style="margin-top:20px;">
				<label>조회기간</label>
				<input type="datetime-local" name="beginDate" style="width:300px;"> ~ <input type="datetime-local" name="endDate" style="width:300px;"> 
				<input class="btn-submit" type="submit" value="검색" style="width:70px;">
			</form>
			...
			<div class="table-type3" style="position: relative; margin-top: 28px; padding-bottom: 15px; border-bottom: 2px #111;">
				<table id="productList" style="width:1400px; position: absolute; left: 50%; transform: translateX(-50%);">
					<colgroup>
						...
						<c:forEach var="AdminOrderDAO" items="${getAdminOrderList}" varStatus="i">
							<tr>
								...
								<form id="searchForm" name="searchForm" action="/w2/updateOdStatus.mdo" method="post">
									...
									<input type="submit" class="btn1" value='변경' style="width:60px; text-align:center">
								</form>
								</td> 
		...
		<div class="paging">
			...
			<c:if test="${ paging.prev }">
				<a href="adminOrder.mdo?page=${ startPage -1 }">처음페이지로</a> 
			</c:if>&nbsp; &nbsp;     
			<c:if test="${ paging.currentPage>1 }">
				<a href="adminOrder.mdo?page=${ paging.currentPage -1 }">이전</a> 
			</c:if>&nbsp;&nbsp;

			<!-- 페이지 버튼 -->
			<c:forEach var="pageNum" begin="${ paging.startPage }" end="${ paging.endPage }">
				<a href="adminOrder.mdo?page=${ pageNum }"
					style="${ (pageNum == paging.currentPage) ? 'color:red; font-style:italic;' : 'color:blue;'}">
					${ pageNum } </a>&nbsp;&nbsp; 
			</c:forEach>

			<c:if test="${ paging.currentPage < paging.endPage }">
				<a href="adminOrder.mdo?page=${ paging.currentPage + 1 }">다음</a>&nbsp;&nbsp;
				</c:if>
			<!-- 다음 버튼 -->
			<c:if test="${ paging.next }">
				<a href="adminOrder.mdo?page=${ paging.endPage +1 }">마지막페이지로</a>
			</c:if>
		</div>
